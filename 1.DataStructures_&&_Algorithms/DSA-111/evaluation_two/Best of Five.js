...
// Calculate Best of Five: Total marks obtained in Top 5 subjectsbestOfFive: function(){}

// Subject Which was not considered in Best of Five: Subject having lowest marksbadSubject: function(){}
...
obj = {
  ...
  badSubject: function () {
    ...
    low_marks = this.marks[0];
    for (i = 0; i <= this.marks.length; i++) {
      if (low_marks > this.marks[i]) {
        low_marks = this.marks[i];
      }
    }
    console.log(low_marks);
  ...
  bestSubject: function () {
    ...
    high_marks = this.marks[0];
    for (i = 0; i <= this.marks.length; i++) {
      if (high_marks < this.marks[i]) {
        high_marks = this.marks[i];
      }
    }
